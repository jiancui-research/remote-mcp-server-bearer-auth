...
async function verifyAccessToken(token: string, provider: DescopeMcpProvider): Promise<AuthInfo> {
	try {
		...
		const scopes = extractAndValidateScopes(
			payload,
			provider.options.verifyTokenOptions?.requiredScopes,
		);
 ...
 */
export function descopeMcpBearerAuth(provider?: DescopeMcpProvider): MiddlewareHandler {
	return async (c: Context, next) => {
		const authProvider =
			provider ||
			new DescopeMcpProvider({}, { env: c.env as unknown as Record<string, string> });
