import type { DescopeMcpProviderOptions } from "./schemas/options.js";
...
class UrlBuilder {
	...
	static construct(base: string, ...paths: string[]): URL {
		try {
			...
			throw new Error(
				`Failed to construct URL with base ${base} and paths ${paths.join(
					", ",
				)}: ${errorMessage}`,
			);
		}
...
export class DescopeMcpProvider {
	...
	constructor(
		{ projectId, managementKey, baseUrl, serverUrl, ...opts }: DescopeMcpProviderOptions = {},
		c?: { env: Record<string, string> },
	) {
	...
	private initializeOptions(
		...
		const dynamicClientRegistrationOptions = {
			...
			attributeScopes:
				opts.dynamicClientRegistrationOptions?.attributeScopes || DEFAULT_ATTRIBUTE_SCOPES,
